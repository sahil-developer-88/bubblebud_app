...
const checkCollision = (characterPos: THREE.Vector3, coinPos: THREE.Vector3, threshold = 1.2) => {
  return Math.abs(characterPos.x - coinPos.x) < threshold && 
...
const GameScene = () => {
  ...
  useFrame(() => {
    if (gameState !== "playing" || motion.beta === undefined) return;
    ...
    if (motion.isCalibrated && motion.calibration.beta !== undefined) {
      ...
      adjustedBeta = motion.beta - motion.calibration.beta;
      ...
      const betaOffset = 5;
      adjustedBeta = motion.beta + betaOffset;
  ...
  useEffect(() => {
    if (gameState === "playing") {
      ...
      console.log("Starting new game - resetting coin counter for proper sequence");
      globalCoinCounter = 0;
  ...
  useFrame((_, delta) => {
    ...
    const missedCoins = updatedCoins.filter(
      coin => coin.position.x <= -2.5 && coin.position.x > -2.75 && !coin.collected
    );
    
    // If any coins were missed, trigger sad face on the character
    if (missedCoins.length > 0 && characterRef.current) {
      // Dispatch a custom event that the Character component can listen for
      const missedCoinEvent = new CustomEvent('coinMissed');
      window.dispatchEvent(missedCoinEvent);
