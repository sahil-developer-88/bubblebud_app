...
export const useBreathingGame = create<BreathingGameState>()(
  subscribeWithSelector((set, get) => ({
    ...
    endGame: () => {
      const { coins, score, startTime, sessionDuration, gameTime } = get();
      ...
      const visibleCoins = coins.filter(coin => !coin.collected);
      ...
      const scorePercentage = totalCoins > 0 ? Math.round((collectedCoins / totalCoins) * 100) : 0;
      
      const actualDuration = startTime ? Math.round((Date.now() - startTime) / 1000) : 0;
      
      // In the display, we'll adjust totalCoins to match the collected coins if score is 100%
      const displayedTotalCoins = scorePercentage === 100 ? collectedCoins : totalCoins;
    ...
    checkSessionEnd: () => {
      const { startTime, sessionDuration, gameState } = get();
    ...
    updateCharacterPosition: (position: Position) => {
      ...
      const { lastBreathDirection, characterPosition: prevPos } = get();
      
      // If moving up significantly, record inhale
      if (position.y > prevPos.y + 0.3 && lastBreathDirection !== "inhale") {
        get().recordBreath("inhale");
      }
      // If moving down significantly, record exhale
      else if (position.y < prevPos.y - 0.3 && lastBreathDirection !== "exhale") {
        get().recordBreath("exhale");
      }
    },
    ...
    updateGameTime: (time: number) => {
      ...
      const shouldEnd = get().checkSessionEnd();
      if (shouldEnd) {
        get().endGame();
      }
    },
    ...
    recordBreath: (direction: "inhale" | "exhale") => {
      ...
      if (newTimestamps.length > 3) {
        ...
        if (recentTimestamps.length >= 2) {
          const timeElapsed = recentTimestamps[recentTimestamps.length - 1] - recentTimestamps[0];
      ...
      console.log(`Breath recorded: ${direction}, Cycles: ${newBreathCycles}, Rate: ${averageBreathRate.toFixed(1)} breaths/min`);
    }
  }))
);
